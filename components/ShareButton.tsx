...
export default function ShareButton({ pollId, pollUid, question, endDate, totalVotes, isEnded }: ShareButtonProps) {
  const [isCopied, setIsCopied] = useState(false);
  
  const pollUrl = useMemo(() => {
    const origin = typeof window !== 'undefined' ? window.location.origin : '';
    return `${origin}/poll/${pollUid || pollId}`;
  }, [pollId, pollUid]);
  
  const shareMessage = `🗳️ ${question}

${isEnded ? '✅ এই জরিপটি সম্পন্ন হয়েছে!' : '⏰ এখনই আপনার মতামত দিন!'}
📊 মোট ভোট: ${toBengaliNumber(totalVotes)} জন
📅 ${isEnded ? 'সম্পন্ন হয়েছে' : 'শেষ হবে'}: ${formatBengaliDate(endDate)}

জরিপে অংশ নিতে ক্লিক করুন:
${pollUrl}

#বাংলাদেশ #নির্বাচন২০২৬ #জনমত`;

  const handleShare = async (platform: string) => {
    const shareUrl = encodeURIComponent(pollUrl);
